...
class FollowUp(models.Model):
    ...
    def is_from_submitter(self):
        ...
        Otherwise we go by the author: ``update_ticket`` only records a user on
        a follow-up when that user is helpdesk staff, so a follow-up with no
        user was either received from the submitter or generated by the
        helpdesk itself -- neither of which was typed by a staff member.
        """
