...
- **QUEUE_EMAIL_BOX_SSL** Set ``True`` or ``False`` to use or not ssl.
...
- **QUEUE_EMAIL_BOX_PASSWORD** The ``password`` of the email account.
  ...
  If ``oauth`` is used, then needs those other settings::
...
- **HELPDESK_IMAP_DEBUG_LEVEL** If using ``imap`` or ``oauth``, set Debug Logging Level for. Default is set to '0', No Debugging.
