...
const toggle_type_affordable_housing = function () {
...
const toggle_affordable_boolean = function () {
...
const toggle_backup_pathway_list_when_ACP_selected = function () {
...
const toggle_require_attachment_when_selected_in_list = function (list_id, pathway_names) {
...
const toggle_alert_field_for_pathway_list = function () {
  const curr_val = $(id.pathway_list).val();
...
$(id.property_list).change(function () {
  ...
  var lists = [id.pathway_list, id.backup_pathway_list, id.new_pathway_list];
  if ($.inArray($(id.property_list).val(), high_perf) === -1) {
    ...
    lists.forEach(function (list) {
      ...
      $(list + ' option').map(function () {
        pathway_options.push(this.value);
      });
...
const toggle_QAH_fields = function () {
...
var ext_values = values.slice(0, 4);
var exempt_values = values.slice(4);

const toggle_ext_exempt = function () {
  if ($(id.ext_or_exempt).val() != '') {
    ...
    var values_to_add = $(id.ext_or_exempt).val() == 'Extension' ? ext_values : exempt_values;
